...
engine = create_engine('sqlite:///product_dbase.sqlite', echo=True)
...
# A metadata is an object container that will store attribute and name of table
...
# User mapper to define components of class as well as table definition together at once
...
# It will rad each excel file in the folder and insert bom into table
def create_table(folder_of_files):
    ...
    for file_name in files:
        ...
        table_name_from_file = os.path.basename(file_name)[:-4]
        print(table_name_from_file)

        # Use table_definition function to define table structure
        table_definition(table_name_from_file)

        # Loop throuh list of lists, each list in create_bom_table.xls_data is a row
        for row in csv_data:
