...
class PipelineRunner(object):
    '''
        ...
        Just run processors in order with passing context from on to another

        You can also log level for processors
    '''
...
class ContourDetection(PipelineProcessor):
    '''
        ...
        and detect with a cv2.findContours method, and then filter off-by
        width and height.
        ...
        image_dir - where to save images(must exitst).
    '''
    ...
    def detect_vehicle(self, fg_mask, context):
        matches = []
    ...
    def __call__(self, context):
        ...
        context['objects'] = self.detect_vehicle(fg_mask, context)
...
class VehicleCounter(PipelineProcessor):
    '''
        ...
        Purpose of this class base on detected object and local cache create
    ...
    def __call__(self, context):
        ...
        # continue vehicles and drop counted pathes
        new_pathes = []
...
class Visualizer(PipelineProcessor):
    ...
    def draw_pathes(self, img, pathes):
        ...
        for i, path in enumerate(pathes):
            ...
            for point in path:
                cv2.circle(img, point, 2, CAR_COLORS[0], -1)
                cv2.polylines(img, [np.int32(path)], False, CAR_COLORS[0], 1)
    ...
    def draw_boxes(self, img, pathes, exit_masks=[]):
        for (i, match) in enumerate(pathes):
            ...
            cv2.rectangle(img, (x, y), (x + w -1, y + h - 1), BOUNDING_BOX_COLOR, 1)
            cv2.circle(img, centroid, 2, CENTROID_COLOR, -1)
        
        return img

    def draw_ui(self, img, vehicle_count, exit_masks=[]):
        # this jus add green mask with opacity to the image
        for exit_mask in exit_masks:
        ...
        cv2.putText(img, ("Vehicle passed: {total}".format(total=vehicle_count)), (30, 30),
