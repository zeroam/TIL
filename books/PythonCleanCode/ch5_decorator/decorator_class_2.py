...
class EventSerializer:
    ...
    def serialize(self, event) -> dict:
        """Get all attribute from ``event``, apply the transformations to
        each attribute, and place it in a dictionary to be returned.
        """
        return {
            field: transformations(getattr(event, field))
            for field, transformations in self.serialization_fields.items()
        }
...
class Serialization:
    ...
    def __init__(self, **transformations):
        ...
        map the attributes of the instance of the class, as serialization time.
        """
